...
class ModelGateway:
    """Capa de preparación para v1.1.0-dev.

    En v1.1.0-dev el gateway convive con endpoints reales ligeros. Por defecto mantiene fallback determinista para que el Space no dependa de PyTorch.
    """
    ...
    def status(self) -> ModelAdapterStatus:
        return ModelAdapterStatus(
            ...
            message=(
                "v1.1.0-dev incluye endpoints de modelos; por defecto usa fallback determinista y Transformers.js/ONNX en navegador."
                if not self.enabled
