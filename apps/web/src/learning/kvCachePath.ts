import type { AttentionBlockType, AttentionMode } from "../types";

export interface PresetAprendizajeKv {
  modo: AttentionMode;
  tokens: number;
  query_heads: number;
  kv_heads: number;
  longitud_contexto: number;
  batch_size: number;
  capas: number;
  dimension: number;
  ventana_swa: number;
  rango_mla: number;
  tipo_bloque: AttentionBlockType;
}

export interface CheckpointAprendizaje {
  id: string;
  titulo: string;
  ...
  preset: PresetAprendizajeKv;
}

export interface PreguntaAprendizaje {
  id: string;
  pregunta: string;
  opciones: string[];
...
export interface RutaAprendizajeKv {
  id: string;
  titulo: string;
  resumen: string;
  duracion_total_minutos: number;
  checkpoints: CheckpointAprendizaje[];
  quiz: PreguntaAprendizaje[];
}

export interface RespuestaQuizAprendizaje {
  correcta: boolean;
  respuesta_correcta: number;
  explicacion: string;
}

export const rutaKvCacheFallback: RutaAprendizajeKv = {
  id: "kv-cache-12-min",
  ...
  checkpoints: [
    {
      ...
      preset: { modo: "causal", tokens: 12, query_heads: 16, kv_heads: 16, longitud_contexto: 32768, batch_size: 1, capas: 24, dimension: 2048, ventana_swa: 4096, rango_mla: 128, tipo_bloque: "mha" }
    },
    {
      ...
      preset: { modo: "causal", tokens: 12, query_heads: 32, kv_heads: 32, longitud_contexto: 131072, batch_size: 1, capas: 32, dimension: 4096, ventana_swa: 4096, rango_mla: 256, tipo_bloque: "mha" }
    },
    {
      id: "mha-vs-gqa",
      ...
      preset: { modo: "gqa", tokens: 12, query_heads: 32, kv_heads: 8, longitud_contexto: 131072, batch_size: 1, capas: 32, dimension: 4096, ventana_swa: 4096, rango_mla: 256, tipo_bloque: "gqa" }
    },
    {
      ...
      preset: { modo: "gqa", tokens: 12, query_heads: 64, kv_heads: 8, longitud_contexto: 1048576, batch_size: 1, capas: 40, dimension: 5120, ventana_swa: 8192, rango_mla: 512, tipo_bloque: "mla" }
    },
    {
      id: "diseno-128k",
      ...
      preset: { modo: "ventana", tokens: 12, query_heads: 32, kv_heads: 8, longitud_contexto: 131072, batch_size: 1, capas: 32, dimension: 4096, ventana_swa: 4096, rango_mla: 256, tipo_bloque: "swa_gqa" }
    }
  ],
  quiz: [
    {
