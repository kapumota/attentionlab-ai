...
- Fase 7 - Calidad visible: badges, CI, checklist de release y quality gate.
- Fase 8 - Validación técnica de KV Cache Estimator: cinco invariantes automatizados, escenarios matemáticos reproducibles y generación de reporte técnico.
- Fase 9 - Segundo Learning Path: recorrido guiado de depuración de agentes con Agent Debugger Timeline.
- Fase 10 - Release pública: tag v1.2.0, `CHANGELOG.md` y `RELEASE_NOTES.md`.

### Roadmap siguiente (post-v1.2.0)

#### Fase 11 - Validación física en GPU real

Medir memoria asignada y reservada para MHA, GQA y SWA en una GPU documentada, usando implementaciones controladas en PyTorch. El protocolo incluirá calentamiento, sincronización CUDA, reinicio de estadísticas de pico, múltiples repeticiones y reporte de variabilidad. Los resultados permitirán evaluar una calibración de la forma `M_físico = α·M_lógico + M_aux`.

#### Fase 12 - Comparación con runtimes de inferencia

Evaluar vLLM, llama.cpp y Hugging Face Transformers bajo configuraciones documentadas de modelo, versión, precisión, batch size, longitud de contexto, patrón de atención y hardware. La comparación distinguirá entre capacidades soportadas de forma nativa y aproximaciones experimentales.

#### Fase 13 - Evaluación de calidad y latencia

Medir memoria pico, latencia, throughput y calidad bajo un protocolo reproducible. Las comparaciones de calidad se realizarán únicamente entre modelos o configuraciones metodológicamente comparables, y los resultados se presentarán como una frontera de Pareto entre memoria, calidad y latencia.

#### Fase 14 - Caso de estudio end-to-end

Documentar un escenario de despliegue con presupuesto de memoria fijo, la configuración recomendada por el estimador, la configuración realmente ejecutada, la diferencia entre predicción y medición, y la calidad, latencia y throughput observados.
