...
Los modelos Transformer establecieron la atención multi-head como componente central de los modelos de lenguaje modernos \cite{vaswani2017attention}. Durante la generación autoregresiva, las claves y valores asociados con los tokens ya procesados se almacenan para evitar recomputación. Esta KV cache reduce cálculo redundante, pero su memoria crece con la longitud de contexto, el número de capas, el lote, la dimensión almacenada y la precisión numérica \cite{shazeer2019fast,pope2022efficiently}.

El problema deja de ser secundario cuando el contexto pasa de miles a cientos de miles o millones de tokens. La KV cache compite con los pesos, las activaciones y los buffers del runtime, limita concurrencia y puede volver inviable una configuración antes de ejecutar una sola consulta. PagedAttention reduce desperdicio y fragmentación durante el servicio \cite{kwon2023efficient}; FlashAttention optimiza accesos a memoria y evita materializaciones innecesarias \cite{dao2022flashattention}; y KIVI cuantiza la cache con una estrategia asimétrica \cite{liu2024kivi}. Estas técnicas son relevantes, pero no eliminan la necesidad de dimensionamiento previo.

Las familias arquitectónicas también modifican el almacenamiento lógico. Multi-query attention comparte claves y valores entre query heads \cite{shazeer2019fast}; GQA introduce un número intermedio de KV heads \cite{ainslie2023gqa}; SWA restringe el contexto activo a una ventana local y puede combinarse con GQA \cite{jiang2023mistral}; y MLA representa información de claves y valores mediante variables latentes de menor dimensión \cite{deepseek2024v2}. Sus implementaciones no son equivalentes, aunque todas alteran factores identificables de la memoria de cache.

La literatura describe cada mecanismo por separado, mientras que una decisión de ingeniería exige responder preguntas operativas: cuánta KV cache requiere una configuración, qué factor produce la reducción, qué combinaciones son algebraicamente compatibles y qué contexto cabe dentro de un presupuesto dado. El objetivo de este trabajo es responder esas preguntas con un marco trazable a código y pruebas, sin presentar estimaciones analíticas como mediciones físicas.

Las contribuciones son: (1) una ecuación base común para MHA, GQA, SWA y cache latente; (2) una factorización adimensional que permite componer mecanismos compatibles; (3) una ecuación inversa para obtener contexto máximo bajo presupuesto; (4) un estudio de sensibilidad con 71 configuraciones y cinco invariantes; (5) una validación física mínima de asignación con PyTorch; y (6) una implementación auditable en Attention AI Lab v1.2.0, con contratos Pydantic, API FastAPI, escenarios versionados, reporte automático y quality gate \cite{lara2026attentionlab}.
